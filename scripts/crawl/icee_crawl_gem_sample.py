...
CSV_FOLDER = '../../data'
CSV_FILE = os.path.join(CSV_FOLDER, 'icee_crawl_v2.csv')
    ...
    while True:
        ...
        for row in rows:
            try:
                ...
                # ✅ '마감' 또는 '종료'가 제목이나 본문에 들어있으면 스킵
                if '마감' in title or '종료' in title or '마감' in content or '종료' in content:
                    print(f'⏩ 제외됨 (마감/종료): {title[:30]}...')
                    continue                # content = content_tag.get_text(separator='\n', strip=True) if content_tag else '본문 없음'
